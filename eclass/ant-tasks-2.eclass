...
# @DESCRIPTION:
# Affects the >=virtual/jdk version set in DEPEND string. Defaults to 1.7, can
...
# @DESCRIPTION:
# Affects the >=virtual/jre version set in DEPEND string. Defaults to 1.7, can
# be overridden from ebuild BEFORE inheriting this eclass.
...
# @FUNCTION: ant-tasks-2_src_unpack
...
ant-tasks-2_src_unpack() {
	[[ -z "${1}" ]] && ant-tasks-2_src_unpack all
	...
	while [[ -n "${1}" ]]; do
		case ${1} in
			base)
				...
				# patch build.xml with our split building modificaiton.
			...
			all)
				ant-tasks-2_src_unpack base jar-dep;;
		esac
		shift
	done
